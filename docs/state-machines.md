...
- 只有交易处于 `COMPLETED` 时，该交易的买家才能发布评价。交易不存在或当前用户
  不可见返回 `404 / RESOURCE_NOT_FOUND`；交易未完成或操作者是卖家等其他参与者
  返回 `403 / FORBIDDEN`。
...
- 评价没有更新或删除端点：发布后内容不可变，作者也不能撤销。
- 评价不产生商品或交易副作用，因此不进入 Product -> Trade 锁序，也不写 Outbox：
  插入是单条语句；`COMPLETED` 是交易终态，读取到的状态不会失效，无需行锁。
- 评价公开可见：已售出商品的详情返回其买家评价；`/users/me/products` 的每件
  商品携带自己收到的买家评价。评价者只公开昵称，不公开学号。
