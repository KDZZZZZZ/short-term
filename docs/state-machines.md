...
评论是交易完成后的产物，发布评论不改变商品或交易状态：

```text
交易 COMPLETED --买家发布--> 存在评论（不可变）
```

权限与唯一性：

- 只有 `(product_id, buyer_id)` 对应的终生唯一交易处于 `COMPLETED` 时，该买家才能
  发布评论。没有交易、交易仍在 `PENDING`/`ACCEPTED` 或已取消时返回
  `403 / FORBIDDEN`；商品不存在返回 `404 / RESOURCE_NOT_FOUND`。
- 评论没有更新或删除端点：发布后内容不可变，作者也不能撤销。
- 交易保证 `(product_id, buyer_id)` 终生最多一笔，评论沿用同一联合唯一：
  数据库层 `product_reviews_buyer_unique` 阻止同一买家对同一商品的第二条评论入库。
  并发重复提交时恰好一个事务提交并返回 `201`，其余返回
  `409 / REVIEW_ALREADY_EXISTS`，不得把唯一约束错误泄漏为 `500`。
- 评论不产生商品或交易副作用，因此不进入 Product -> Trade 锁序，也不写 Outbox：
  创建事务只读取商品存在性与已完成交易，再插入评论行；`COMPLETED` 是交易终态，
  事务内读取的结果之后不会失效，无需对商品或交易行加锁。
