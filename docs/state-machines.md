...
- 联系方式出现在三类位置，可见范围各不相同：商品详情的卖家联系方式对所有已认证用户可见；交易表示中的双方联系方式仅交易双方可见（交易读取端点本就只对交易方开放），买家可能未填写，对应字段为 `null`；公开用户资料不包含任何联系方式。
- 公开用户资料（`GET /users/{userId}`）只有昵称和该用户作为卖家的评分平均值，不含学号和联系方式。
...
评论是商品上的独立 UGC，不依赖交易，发布评论也不改变商品或交易状态：

```text
任意现存商品 --任意用户发布--> 存在评论（不可变）
```

权限与数量：

- 任何已认证用户都可以对任何现存商品发布评论：不要求存在交易，不限商品状态，
  用户也可以评论自己发布的商品。商品不存在返回 `404 / RESOURCE_NOT_FOUND`。
- 每个用户对同一商品可以发布多条评论；评论之间相互独立，服务端不仲裁内容重复。
- 评论没有更新或删除端点：发布后内容不可变，作者也不能撤销。
- 评论不产生商品或交易副作用，因此不进入 Product -> Trade 锁序，也不写 Outbox：
  插入是单条语句，商品存在性由外键约束兜底；商品没有任何删除路径，已提交的
  评论不会因商品状态变化而失效。
- 评论列表对所有已认证用户可见，与商品状态无关；评论者只公开昵称，不公开学号。

## 买家评价

买家评价是交易完成后买家对交易的文字评价，与公开的用户评论相互独立：

```text
交易 COMPLETED --买家发布--> 存在买家评价（不可变）
```

权限与唯一性：

- 评价包含 1 至 5 的整数评分（必填）和可选文字。只有交易处于 `COMPLETED` 时，
  该交易的买家才能发布评价。交易不存在或当前用户不可见返回
  `404 / RESOURCE_NOT_FOUND`；交易未完成或操作者是卖家等其他参与者返回
  `403 / FORBIDDEN`。
- 每笔交易最多一条评价，数据库层 `trade_reviews_trade_unique` 阻止第二条入库；
  并发重复提交时恰好一个事务提交并返回 `201`，其余返回
  `409 / TRADE_REVIEW_ALREADY_EXISTS`，不得把唯一约束错误泄漏为 `500`。
  由于每个商品终生最多一笔已完成交易，每个商品也最多一条买家评价。
- 评价没有更新或删除端点：发布后评分与文字不可变，作者也不能撤销。
- 评价不产生商品或交易副作用，因此不进入 Product -> Trade 锁序，也不写 Outbox：
  插入是单条语句；`COMPLETED` 是交易终态，读取到的状态不会失效，无需行锁。
- 评价公开可见：已售出商品的详情返回其买家评价（含评分）；`/users/me/products`
  的每件商品携带自己收到的买家评价。评价者只公开昵称，不公开学号。
- 卖家的公开平均分是其作为卖家收到的全部评分的平均值，固定两位小数；
  没有任何评分的用户为 `null`。平均值由 Marketplace Service 从评价表聚合，
  公开于商品详情的卖家信息和本人资料中，属于展示投影而非独立资源。
