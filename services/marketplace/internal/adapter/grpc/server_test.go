...
func newHarnessWithConversationVerifier(t *testing.T, ids application.IDGenerator, conversations application.ConversationVerifier) harness {
	...
	comments, err := application.NewCommentService(postgres.NewCommentRepository(pool), ids, system.Clock{})
	if err != nil {
		t.Fatalf("NewCommentService: %v", err)
	}
	tradeReviews, err := application.NewTradeReviewService(
		postgres.NewTradeRepository(pool), postgres.NewTradeReviewRepository(pool), ids, system.Clock{},
	)
	if err != nil {
		t.Fatalf("NewTradeReviewService: %v", err)
	}
